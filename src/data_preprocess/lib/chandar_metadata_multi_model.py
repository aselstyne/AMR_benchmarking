    ...
    for index, row in data.iterrows():
        for column in data.columns:
            ...
            if cell_value==1:
                ...
                pheno_summary=pd.read_csv('./data/NCBI/meta/'+str(level)+'_genomeNumber/log_' + str(species.replace(" ", "_")) + '_pheno_summary' + '.txt', index_col=0,sep="\t")

                genome_count=pheno_summary.at[anti,'Resistant']+pheno_summary.at[anti,'Susceptible']
                data_size.at[species,anti]=genome_count

    print(data_size)
    data_size.to_csv('./data/NCBI/meta/'+str(level)+'_genomeNumber/multi-species-antibiotic.csv', sep="\t")
